...
# Function to create VecStim from spike times
def create_vecstim(spike_times):
    vec = h.Vector(spike_times)
    vec_stim = h.VecStim()
    vec_stim.play(vec)
    return vec_stim

# Function to add VecStim inputs to the network
def add_vecstim_inputs(pop_name, spike_times_trials, netParams, target_pop):
    for i, trial in enumerate(spike_times_trials):
        vec_stim = create_vecstim(trial)
        stim_source = f'vecStim_{pop_name}_{i}'
        netParams.stimSourceParams[stim_source] = {'type': 'VecStim', 'timeVec': vec_stim}
        netParams.stimTargetParams[stim_source] = {
            'source': stim_source,
            'conds': {'pop': target_pop},
            'weight': 0.01,
            'delay': 5,
            'sec': 'soma',
            'loc': 0.5}
...
for trial_index in range(MinTrials):
    decre_spike_times_trials = []
    incre_spike_times_trials = []

    # Collect spike times for the current trial index
    for neuron in pop_decre_dict:
        neuron_lists = pop_decre_dict[neuron]
        decre_spike_times_trials.append(list(neuron_lists[trial_index]))

    for neuron in pop_incre_dict:
        neuron_lists = pop_incre_dict[neuron]
        incre_spike_times_trials.append(list(neuron_lists[trial_index]))

    # Adjust spike times if necessary
    ...
    # Add VecStim inputs to the network
    add_vecstim_inputs('pop_incre', incre_spike_times_trials, netParams, 'TargetNeuron')
    add_vecstim_inputs('pop_decre', decre_spike_times_trials, netParams, 'TargetNeuron')

    # Simulation configuration
    simConfig = specs.SimConfig()
    simConfig.duration = 1 * 1e3  # Simulation duration in ms
    simConfig.dt = 0.025  # Time step
    simConfig.recordTraces = {'V_soma': {'sec': 'soma', 'loc': 0.5, 'var': 'v'}}  # Record membrane potential from soma
    simConfig.recordStep = 0.1  # Step size in ms to save data
    simConfig.filename = f'output_trial_{trial_index}'  # Output filename
    simConfig.saveJson = True  # Save params, cell and sim output to JSON file
    simConfig.analysis['plotRaster'] = True  # Plot a raster

    # Create and run the simulation
    sim.createSimulateAnalyze(netParams=netParams, simConfig=simConfig)

    # Clear VecStim sources and targets for the next trial
    netParams.stimSourceParams.clear()
    netParams.stimTargetParams.clear()
